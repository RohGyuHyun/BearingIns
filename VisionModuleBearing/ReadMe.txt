========================================================================
    MFC 라이브러리 : VisionModuleBearing 프로젝트 개요
========================================================================


응용 프로그램 마법사에서 이 VisionModuleBearing DLL을 만들었습니다.  이 DLL은
MFC의 기본 사용법을 보여 줄 뿐만 아니라 DLL을 작성하기 위한 기본 구조를 제공합니다.

이 파일에는 VisionModuleBearing DLL을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.

VisionModuleBearing.vcproj
    응용 프로그램 마법사를 사용하여 생성한 VC++ 프로젝트의 기본 프로젝트 파일입니다.
    파일을 생성한 Visual C++ 버전에 대한 정보와
    응용 프로그램 마법사를 사용하여 선택한 플랫폼, 구성 및 프로젝트 기능에 대한
    정보가 들어 있습니다.

VisionModuleBearing.h
    해당 DLL의 기본 헤더 파일이며,  CVisionModuleBearingApp
    클래스가 선언되어 있습니다.

VisionModuleBearing.cpp
    기본 DLL 소스 파일입니다.  CVisionModuleBearingApp 클래스가 포함되어 있습니다.

VisionModuleBearing.rc
    프로그램에서 사용하는 모든 Microsoft Windows 리소스의
    목록입니다.  여기에는 RES 하위 디렉터리에 저장된 아이콘, 비트맵 및 커서가
    포함됩니다.  이 파일은 Microsoft Visual C++에서 직접 편집할 수
    있습니다.

res\VisionModuleBearing.rc2
    이 파일에는 Microsoft Visual C++ 이외의 다른 도구에서 편집한 리소스가
    포함되어 있습니다.  리소스 편집기로 편집할 수 없는 리소스는
    모두 이 파일 안에 배치해야 합니다.

VisionModuleBearing.def
    이 파일에는 Microsoft Windows에서 실행되기 위해 제공해야 하는
    DLL에 대한 정보가 포함되어 있으며,  DLL의 이름 및 설명과 같은
    매개 변수가 정의되어 있습니다.  또한 DLL에서
    함수도 내보냅니다.

/////////////////////////////////////////////////////////////////////////////
기타 표준 파일:

StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일인 VisionModuleBearing.pch와
    미리 컴파일된 형식(PCT) 파일인 StdAfx.obj를 빌드하는 데 사용됩니다.

Resource.h
    새 리소스 ID를 정의하는 표준 헤더 파일입니다.
    Microsoft Visual C++에서 이 파일을 읽고 업데이트합니다.

/////////////////////////////////////////////////////////////////////////////
기타 참고:

응용 프로그램 마법사에서 사용하는 "TODO:" 주석은 사용자가 추가하거나 사용자 지정해야 하는
소스 코드 부분을 나타냅니다.

/////////////////////////////////////////////////////////////////////////////
